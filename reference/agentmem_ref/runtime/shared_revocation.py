"""Governed shared-domain membership authority and revocation propagation.

Shared-domain membership controls who may recall retained memory from a shared
isolation domain. Changing that membership is an ``authority_change``
consequence, not ordinary runtime configuration.

The embedding host remains responsible for authenticating ``principal_ref`` on
recall. This module governs Agent Memory's recorded membership state and the
propagation consequences of revocation; it does not introduce a local identity
provider.
...
turn that requirement into a normal ``allow`` outcome. A bound external
verification may satisfy the authority requirement for the exact membership
consequence while the canonical PAMA decision remains
...
def bootstrap_shared_domain_members(
    ...
    Runtime membership changes after initialization belong through
    ``commit_shared_domain_membership_change``. Keeping bootstrap explicit stops
    fixture construction from looking like the governed production path.
    """
...
def commit_shared_domain_membership_change(
    ...
    *,
    ...
    The durable consequence binds the exact domain, prior and resulting member
    sets, proposal id, state snapshot, canonical PAMA requirement, and a
    separately validated external authority record.

    PAMA remains ``require_external_verification`` for this A5 change even when
    valid external authority is present. The external authority satisfies the
    consequence requirement; it does not rewrite PAMA into ``allow``.
    """
    ...
    # Evidence can support the proposition under review, but it cannot lower the
    # A5 authority floor. The attestation is therefore intentionally omitted from
    # PAMA evaluation and validated separately below.
    ...
    # The adapter-owned maps remain part of #363's persistence-contract cleanup.
    # The current reference restart profile already snapshots both membership and
    # extension_state, so this authority consequence remains restart-safe today.
...
def propagate_shared_membership_revocation(
    ...
    *,
    ...
    This function deliberately does not mutate shared-domain membership. The
    authority change must already have committed through
    ``commit_shared_domain_membership_change``. The current adapter membership
    must equal ``remaining_members`` or propagation refuses.
    """
    ...
    current_members = current_shared_domain_members(adapter, domain_ref)
    expected_members = _members(remaining_members)
